...
scfc_sync_range = np.linspace(df_scfcs['scfc_sync'].min(), df_scfcs['scfc_sync'].max(), 100)
grid_seq, grid_sync = np.meshgrid(scfc_seq_range, scfc_sync_range)

# --- Fit additive GAM (raw discharge) ---
X_add = df_scfcs[['scfc_seq', 'scfc_sync']].values
...
X_tensor = df_log[['scfc_seq', 'scfc_sync']].values
...
# --- (a) Partial effect on Q_flume1 for scfc_sync (term 1, additive) ---
X_sync = np.column_stack([np.repeat(df_scfcs['scfc_seq'].mean(), 100), scfc_sync_range])
pdep_sync, confi_sync = gam_add.partial_dependence(term=1, X=X_sync, width=0.95)
axs[0].plot(scfc_sync_range, pdep_sync, color='blue', label='Partial effect')
axs[0].fill_between(scfc_sync_range, confi_sync[:, 0], confi_sync[:, 1], alpha=0.3, color='blue')
...
X_seq = np.column_stack([scfc_seq_range, np.repeat(df_scfcs['scfc_sync'].mean(), 100)])
...
X_grid = np.column_stack([grid_seq.ravel(), grid_sync.ravel()])
...
cf = axs[2].contourf(grid_seq, grid_sync, partial_effect_tensor, levels=20, cmap='RdBu_r', alpha=0.8)
axs[2].contour(grid_seq, grid_sync, partial_effect_tensor, levels=10, colors='black', linewidths=0.5)
axs[2].scatter(df_scfcs['scfc_seq'], df_scfcs['scfc_sync'], c='gray', s=55, edgecolor='black', label='Data points')
