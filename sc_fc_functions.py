...
def euclidean_distance(flume_pair, df_flume_coordinates):
    flume1 = df_flume_coordinates[df_flume_coordinates[FLUME_COL] == flume_pair[0]]
    flume2 = df_flume_coordinates[df_flume_coordinates[FLUME_COL] == flume_pair[1]]
...
def compute_sc_sim(df_flume_coordinates, df_contributing_area):
    ...
    """
    edges_eucl_distance = list(permutations(df_flume_coordinates[FLUME_COL], 2))
    ...
    for i, row in df_sc_sim.iterrows():
        area1 = df_contributing_area[df_contributing_area[FLUME_COL] == row['flume_1']][AREA_COL].iloc[0]
        area2 = df_contributing_area[df_contributing_area[FLUME_COL] == row['flume_2']][AREA_COL].iloc[0]
